...
pub(super) type Coefficient = Scalar;
pub(super) type Value = Scalar;
pub(super) type ValueCommitment = RistrettoPoint;
pub(super) type CoefficientCommitment = RistrettoPoint;
...
impl Polynomial {
    pub(super) fn generate<R: RngCore + CryptoRng>(rng: &mut R, degree: u16) -> Self {
    ...
    pub(super) fn evaluate(&self, x: &Value) -> Value {
        let mut value =
...
pub struct PolynomialCommitment {
    pub(super) coefficients_commitments: Vec<CoefficientCommitment>,
}

impl PolynomialCommitment {
    pub(super) fn commit(polynomial: &Polynomial) -> Self {
    ...
    pub(super) fn evaluate(&self, identifier: &Value) -> ValueCommitment {
        let i = identifier;
