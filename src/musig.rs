...
pub fn aggregate_public_key_from_slice<'a>(
    public_keys: &'a mut [PublicKey],
) -> Option<AggregatePublicKeySlice<'a, PublicKey>> {
    if public_keys.len() == 1 {
        return None;
    }
    public_keys.sort_unstable();
    if public_keys.windows(2).any(|x| x[0] == x[1]) {
        return None;
    }
...
/// Aggregate public keys stored in a mutable slice
pub fn aggregate_public_key_from_refs_slice<'a>(
...
impl Commitment {
    ...
    where
        I: IntoIterator<Item = CompressedRistretto>,
    {
        let mut t = Transcript::new(b"MuSig-commitment");
        for R0 in R.into_iter() {
            t.commit_point(b"sign:R", &R0);
        }
...
impl Reveal {
    ...
    fn into_points(&self) -> SignatureResult<RevealedPoints> {
        ...
        let a = self
            .iter_points()
            .map(|x| x.decompress().ok_or(SignatureError::PointDecompressionError))
...
impl CoR {
    ...
    */
    ...
    fn set_revealed(&mut self, reveal: Reveal) -> SignatureResult<()> {
        ...
        match self.clone() {
            // TODO: Remove .clone() here with #![feature(nll)]
            ...
            CoR::Reveal(reveal_old) => {
                if reveal_old == reveal {
                    Ok(())
                } else {
                    // TODO: Restore *R_old here with #![feature(nll)]
...
impl<T: SigningTranscript + Clone, S> MuSig<T, S> {
    ...
    fn compute_public_key(&self, require_reveal: bool) -> PublicKey {
        ...
        let point = self
            .public_keys(require_reveal)
            .map(|pk| compute_weighting(t0.clone(), pk) * pk.as_point())
            .sum();
        PublicKey::from_point(point)
    }
    ...
    fn iter_Rs(&self) -> impl Iterator<Item = (&PublicKey, &RevealedPoints)> {
        self.Rs.iter().filter_map(|(pk, cor)| match cor {
            ...
            CoR::Cosigned { .. } => {
                panic!("Internal error, compute_R called during cosigning phase.")
            },
    ...
    T: SigningTranscript + Clone,
{
    ...
    pub fn new(keypair: K, t: T) -> MuSig<T, CommitStage<K>> {
        ...
        let R_me_points: ArrayVec<RistrettoPoint, REWINDS> =
            r_me.iter().map(|r_me_i| r_me_i * B).collect();
    ...
    T: SigningTranscript + Clone,
{
    ...
    pub fn add_their_reveal(&mut self, them: PublicKey, theirs: Reveal) -> SignatureResult<()> {
        match self.Rs.entry(them) {
            ...
            Entry::Occupied(mut o) => o.get_mut().set_revealed(theirs),
        }
    }
    ...
    pub fn cosign_stage(mut self) -> MuSig<T, CosignStage> {
        ...
        *(Rs.get_mut(&self.stage.keypair.borrow().public)
            .expect("Rs known to contain this public; qed")) = CoR::Cosigned { s: s_me };
...
impl<T: SigningTranscript + Clone> MuSig<T, CosignStage> {
    ...
    pub fn sign(&self) -> Option<Signature> {
        ...
        if self.uncosigned().last().is_some() {
            return None;
        }
        let s: Scalar = self
            .Rs
            ...
            .filter_map(|(_pk, cor)| match cor {
                CoR::Commit(_) => None,
                CoR::Reveal(_) => {
                    panic!("Internal error, MuSig<T,CosignStage>::uncosigned broken.")
                },
                CoR::Cosigned { s, .. } => Some(s),
                CoR::Collect { .. } => panic!("Collect found in Cosign phase."),
            })
    ...
    fn multi_signature() {
        ...
        for i in 0..commits.len() {
            ...
            for j in commits.iter_mut() {
                assert!(
                    j.add_their_commitment(keypairs[i].public.clone(), r).is_ok()
                        != (r == j.our_commitment())
                );
            }
        }
