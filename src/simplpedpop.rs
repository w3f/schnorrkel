...
use crate::{aead::make_aead, context::SigningTranscript, PublicKey, SecretKey, Signature};
...
pub mod round3 {
    use super::{
        round1, round2, Certificate, GroupPublicKey, GroupPublicKeyShare, Identifier, Identifiers,
        Parameters, SecretPolynomial, SecretShare, TotalSecretShare, GENERATOR,
    };
    use crate::{
        context::SigningTranscript,
        errors::{DKGError, DKGResult},
    ...
    pub fn run<T: SigningTranscript + Clone>(
        ...
        verify_round2_public_messages(round2_public_messages, round2_public_data)?;
        ...
        let (group_public_key, group_public_key_shares) =
            generate_public_data(round2_public_data, round1_public_data)?;
    ...
    fn verify_round2_public_messages<T: SigningTranscript + Clone>(
        ...
        verify_batch(
            vec![
                round2_public_data.transcript.clone();
                round2_public_data.identifiers.others_identifiers.len()
            ],
            &round2_public_messages
                .iter()
                .map(|(id, msg)| {
    ...
    fn verify_round2_private_messages<T: SigningTranscript + Clone>(
        round2_public_data: &round2::PublicData<T>,
        round2_private_messages: &BTreeMap<Identifier, round2::PrivateMessage>,
        secret: &Scalar,
    ...
    fn generate_public_data<T: SigningTranscript + Clone>(
        ...
        let own_group_public_key_share = total_secret_polynomial_commitment
            .evaluate(&round2_public_data.identifiers.own_identifier.0);
...
mod tests {
    ...
    fn test_invalid_secret_share_error_in_round3() {
        ...
        match result {
            ...
            Err(e) => assert_eq!(
                e,
                DKGError::InvalidSecretShare(participants_sets_of_participants[1].own_identifier),
